 ...
 ###########################################################################
  ...
  # find weather at Bethel airport in both datasets    
    NOAAmeta[ which( NOAAmeta$id == "USC00503672" ), ]
    
    sbNOAA <- subset( NOAAwx, id == "USC00503672" & Element == "SNWD" )
    nrow( sbNOAA )  
    
    for( i in 1:length( meta[,1] ) ){
     
       indx <- which( unlist( meta$sids[i] ) == "USC00503672 6" )
       if( length( indx ) > 0 ){
        print( i ) 
       }
    }
    ...
    meta[ 48, ]
    sbwx <- subset( wx, dbID == 48 )
    nrow( sbwx )       
    
    plot( as.Date(sbwx$date), sbwx$snwd )
    points( as.Date(sbNOAA$date), (sbNOAA$Value/25.4), type = "l", col="red" )
