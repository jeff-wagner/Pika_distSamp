...
m14 <- distsamp(~scale(search.speed) ~scale(meanNDVI),
                umf, keyfun="hazard", output="density", unitsOut="kmsq")
m15 <- distsamp(~scale(search.speed) ~scale(meanNDVIcell),
                umf, keyfun="hazard", output="density", unitsOut="kmsq")
m16 <- distsamp(~scale(search.speed) ~scale(snowDepthMean), 
                umf, keyfun="hazard", output="density", unitsOut="kmsq")
...
full <- distsamp(~scale(search.speed) ~scale(shrubCover) + scale(summer.pcpn.mm) +
...
models <- get.models(modelList, subset=TRUE, clust)
